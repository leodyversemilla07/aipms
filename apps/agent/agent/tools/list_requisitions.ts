...
export default defineTool({
  ...
  async execute(input) {
    return await trpcQuery("requisition", "list", {
      q: input.status,
      page: input.page ?? 1,
  ...
  toModelOutput(result: RelayPayload) {
    ...
    if (!items || !Array.isArray(items))
      return { type: "text", value: "No requisitions found." }
    const summary = items
      .map(
        (req: RelayPayload) =>
          `- ID: ${req.id}, Status: ${req.status}, Title: ${req.title || "Untitled"}`
      )
      .join("\n")
    return {
      type: "text",
      value: `Found ${items.length} requisitions.\n${summary}`,
    }
  },
})
