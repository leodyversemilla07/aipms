...
test("requisition → PO → receipt, over-receipt refused", async ({ page }) => {
  ...
  if (!mine) throw new Error("created requisition appears in the API list")
  ...
  const poNumber = ((await card.getByText(/PO \S+ issued/).innerText()).match(
    /PO-\d+/
  ) ?? [])[0]
  if (!poNumber) throw new Error("issued PO number not surfaced")
  ...
  await page.getByRole("option", { name: poNumber }).click()
